...
API_TOKEN = os.getenv("OBMEN_BOT_TOKEN", "8458444274:AAFNBpfAPwMsrmxIIurI_W1HKVVe8dbuPic")
...
async def buy_start(message: types.Message, state: FSMContext):
    ...
    buttons = [types.InlineKeyboardButton(cur, callback_data=f"buy_cur|{cur}") for cur in available]
    for i in range(0, len(buttons), 2):
        kb.row(*buttons[i:i+2])
    kb.row(types.InlineKeyboardButton("⏹️ Bekor qilish", callback_data="buy_cur|cancel"))
...
async def sell_start(message: types.Message, state: FSMContext):
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    buttons = [types.InlineKeyboardButton(cur, callback_data=f"sell_cur|{cur}") for cur in cur_list]
    for i in range(0, len(buttons), 2):
        kb.row(*buttons[i:i+2])
    kb.row(types.InlineKeyboardButton("⏹️ Bekor qilish", callback_data="sell_cur|cancel"))
...
# Buy FSM
@dp.callback_query_handler(lambda c: c.data.startswith("buy_cur|"), state=BuyFSM.choose_currency)
async def buy_choose_currency(call: types.CallbackQuery, state: FSMContext):
    cur = call.data.split("|")[1]
    await call.answer()
    if cur == "cancel":
        await state.finish()
        return await call.message.answer("Bekor qilindi.", reply_markup=main_menu_kb())
    if cur not in currencies:
        return await call.message.answer("Bunday valyuta yo'q.")
    await state.update_data(currency=cur)
    await call.message.answer("Miqdorni kiriting:", reply_markup=back_kb())
    await BuyFSM.next()
...
# Sell FSM
@dp.callback_query_handler(lambda c: c.data.startswith("sell_cur|"), state=SellFSM.choose_currency)
async def sell_choose_currency(call: types.CallbackQuery, state: FSMContext):
    cur = call.data.split("|")[1]
    await call.answer()
    if cur == "cancel":
        await state.finish()
